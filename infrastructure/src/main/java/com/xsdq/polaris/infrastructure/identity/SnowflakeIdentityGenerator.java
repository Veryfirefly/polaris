...
/**
 * A simple snowflake distributed id generator algorithm from
 * <a href="https://pdai.tech/md/algorithm/alg-domain-id-snowflake.html">https://pdai.tech/md/algorithm/alg-domain-id-snowflake.html</a>
 *
 * @author XiaoYu
...
public class SnowflakeIdentityGenerator implements IdentityGenerator {
	...
	private static final long maxWorkerId = -1L ^ (-1L << workerIdBits); // 支持的最大机器id，结果是31 (这个移位算法可以很快的计算出几位二进制数所能表示的最大十进制数)
	...
	private static final long timestampLeftShift = sequenceBits + workerIdBits + datacenterIdBits; // 时间截向左移22位(5+5+12)
	private static final long sequenceMask = -1L ^ (-1L << sequenceBits); // 生成序列的掩码，这里为4095 (0b111111111111=0xfff=4095)
