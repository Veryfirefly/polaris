...
public class Role extends BaseEntity {
	...
	public void revoke(PermissionId permissionId) {
		if (!permissionIds.remove(permissionId))
			throw new IllegalStateException("角色未拥有该权限");
	...
	public void disable() {
		if (this.status == RoleStatus.DISABLED)
			throw new IllegalStateException("角色已禁止使用");
	...
	public void enable() {
		if (this.status == RoleStatus.ENABLED)
			throw new IllegalStateException("角色已启用");
	...
	public static Role create(RoleId roleId,
			TenantId tenantId,
			String name,
			RoleEntity entity,
			String description,
			Set<PermissionId> permissionIds) {
		return new Role(roleId, tenantId, name, entity, description,
				RoleStatus.ENABLED, permissionIds, LocalDateTime.now(), LocalDateTime.now());
	}

	public static Role reconstitute(RoleId roleId,
			TenantId tenantId,
			String name,
			RoleEntity entity,
			String description,
			...
			LocalDateTime createTime,
			LocalDateTime updateTime) {
		return new Role(roleId, tenantId, name, entity, description, status, permissionIds, createTime, updateTime);
	}
}
