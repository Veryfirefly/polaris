...
public class User extends BaseEntity {
	...
	public void removeRole(RoleId roleId) {
		...
		if (!roleIds.remove(roleId))
			throw new IllegalStateException("用户未拥有该角色");
	...
	public void frozen() {
		if (this.status == UserStatus.FROZEN)
			throw new IllegalStateException("用户状态已为冻结中");
	...
	public void normal() {
		if (this.status == UserStatus.NORMAL)
			throw new IllegalStateException("用户状态已为正常");
	...
	public static User create(UserId userId,
		...
		return new User(userId, tenantId, nickname, account, password, email, null, null, null,
				UserStatus.INACTIVE, assignRoleIds, LocalDateTime.now(), LocalDateTime.now());
	}
	...
	public static User reconstitute(UserId userId,
									...
									LocalDateTime createTime,
									LocalDateTime updateTime) {
		return new User(userId, tenantId, nickname, account, password, email, concat, address, avatar,
				status, roleIds, createTime, updateTime);
	}
