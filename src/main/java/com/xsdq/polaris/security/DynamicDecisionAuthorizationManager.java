...
public class DynamicDecisionAuthorizationManager implements AuthorizationManager<RequestAuthorizationContext> {
	...
	private static final Logger log = LoggerFactory.getLogger(DynamicDecisionAuthorizationManager.class);

	private final MenuService menuService;
	private final AuthenticationTrustResolver trustResolver = new AuthenticationTrustResolverImpl();

	public DynamicDecisionAuthorizationManager(MenuService menuService) {
	...
	public AuthorizationDecision check(Supplier<Authentication> authenticationSupplier, RequestAuthorizationContext context) {
		...
		List<Permission> permissions = menuService.listPermissions();
		...
		if (log.isDebugEnabled()) {
			log.debug("No permissions match, request: [{} {}].",
