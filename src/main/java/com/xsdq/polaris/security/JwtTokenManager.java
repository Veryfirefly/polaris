...
import com.xsdq.polaris.cache.RedisCacheService;
...
public class JwtTokenManager implements TokenManager<PolarisUserDetails> {
	...
	private final Supplier<String> createUuidSupplier = () -> UUID.randomUUID().toString().replace("-", "");

	public JwtTokenManager(PolarisSecurityProperties securityProperties, RedisCacheService redisCacheService) {
		this.timeToLive = securityProperties.getToken().getTimeToLive();
		this.timeToRefresh = securityProperties.getToken().getTimeToRefresh();
		this.secretKey = securityProperties.getToken().signingKey();
		this.authHeaderName = securityProperties.getToken().getHeaderName();
	...
	public PolarisUserDetails getUserDetails(String token) throws JwtException {
		...
		if (!StringUtils.hasText(uuid) || tenantId == null)
			throw new JwtException("Invalid token");

		PolarisUserDetails userDetails = redisCacheService.get(CACHE_KEY_TEMPLATE.formatted(tenantId, uuid));
		if (userDetails != null)
			renewalToken(userDetails);
	...
	@Override
	public CreatedToken createToken(PolarisUserDetails userDetails) throws JwtException {
		...
		redisCacheService.put(CACHE_KEY_TEMPLATE.formatted(userDetails.tenantId(), uuid), userDetails, timeToLive);

		if (log.isDebugEnabled()) {
			log.debug("所属租户'{}-{}'下的用户'{}'创建了jwt token: {}", userDetails.tenantId(), userDetails.tenantId(),
					userDetails.getUsername(), token);
		}

		ApplicationUtils.publishEvent(new LoginHistoryEvent(userDetails, LoginStatus.LOGGED_IN, Utils.currentClientIp()));

		return new CreatedToken(token, timeToLive.toMillis(), userDetails.tenantId());
	}
	...
	public void removeUserDetails(PolarisUserDetails userDetails) throws JwtException {
		...
		redisCacheService.evict(CACHE_KEY_TEMPLATE.formatted(userDetails.tenantId(), userDetails.getIdentifier()));
	}
	...
	public PolarisUserDetails renewalToken(PolarisUserDetails userDetails) throws JwtException {
		...
		if (userDetails.canRenewal(timeToRefresh)) {
			...
			ApplicationUtils.publishEvent(new LoginHistoryEvent(userDetails, LoginStatus.RENEWAL, Utils.currentClientIp()));

			if (log.isDebugEnabled()) {
				log.debug("'{}-{}' token to be renewal to {}", userDetails.tenantId(), userDetails.getUser(),
