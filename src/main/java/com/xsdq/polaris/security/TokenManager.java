...
public interface TokenManager<T extends UserDetails> {
	 ...
	 */
	CreatedToken createToken(T userDetails) throws JwtException;

	/**
	 * Returns the logged-in credential information associated with the tenant.
	 * <strong> Key information requires anonymization.</strong>
	 *
	...
	/**
	 * Removes an existing credential using an access token;
	 * if the credential does not exist, no action is taken.
	 *
	...
	/**
	 * Renews the validity of a credential.
	 * If the credential's expiration date falls within a specified future timeframe,
	 * the associated Access Token is renewed; otherwise, it is considered expired.
	 *
	 * @param userDetails Logged-in Credential Information for the Tenant
